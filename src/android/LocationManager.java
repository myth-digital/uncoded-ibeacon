...
public class LocationManager extends CordovaPlugin implements BeaconConsumer {
  ...
  private void tryToRequestMarshmallowLocationPermission() {
    ...
    try {
      final Integer locationPermissionCheckResult = (Integer) checkSelfPermissionMethod.invoke(
        activity,
      ...
      Log.i(
        TAG,
        "Permission check result for ACCESS_FINE_LOCATION: " +
        String.valueOf(locationPermissionCheckResult)
      );
      ...
      if (locationPermissionCheckResult == PackageManager.PERMISSION_GRANTED) {
        Log.i(
          TAG,
          "Permission for ACCESS_FINE_LOCATION has already been granted."
        );
      } else {
        final AlertDialog.Builder builder = new AlertDialog.Builder(activity);
        builder.setTitle("Alpha Victor needs location access");
        builder.setMessage(
      ...
      final Integer bluetoothPermissionCheckResult = (Integer) checkSelfPermissionMethod.invoke(
        activity,
      ...
      Log.i(
        TAG,
        "Permission check result for BLUETOOTH_SCAN: " +
        String.valueOf(bluetoothPermissionCheckResult)
      );

      if (bluetoothPermissionCheckResult == PackageManager.PERMISSION_GRANTED) {
        Log.i(TAG, "Permission for BLUETOOTH_SCAN has already been granted.");
      } else {
        final AlertDialog.Builder builder = new AlertDialog.Builder(activity);
        builder.setTitle("Alpha Victor needs Bluetooth access");
        builder.setMessage(
          "Please grant bluetooth access so this app can detect beacons."
        );
        builder.setPositiveButton(android.R.string.ok, null);
        builder.setOnDismissListener(
          new DialogInterface.OnDismissListener() {
